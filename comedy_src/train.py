...
test_edge_neg_ = test_edge_neg[:int(len(test_edge_pos) * anomaly_radio)]
...
model_ = Model(num_nodes,node_dim,hidden_size,args.threshold,device).to(device)
...
f = open(str(dataset)+str(ratio)+str(args.sampling)+'.txt', 'a+')
f.write(str(args)+'\n')
#
#
auc=[]
