...
class Model(nn.Module):
    def __init__(self, num_nodes, embedding_dims, hidden_size,k, device, gcn=False):
        ...
        self.updater = nn.GRU(input_size=embedding_dims*2, hidden_size=embedding_dims*2, num_layers=1,
                              batch_first=True)
        self.feat=0
        self.graph_remain=0
        self.k=k
    ...
    def forward(self, edge,label,flag='test'):
        ...
        for i in range(2):
            ...
            eff_tar=self.linear_node(torch.Tensor([degree_node]))
            ...
            for neigh in in_neighbors:
                ...
                eff_neigh = self.linear_node(torch.Tensor([degree_neigh]))
        ...
        return torch.tanh(combined_all)
        # return combined_all